	...
	for (my $i=0; $i < @{$ids}; $i++) {
		...
		if ($ids->[$i] =~ m/cpd\d\d\d\d\d/ && defined($idHash->{$ids->[$i]})) {
			...
			if (defined($cpdAlsHash->{$ids->[$i]})) {
				$names = [];
				for (my $i=0; $i < @{$cpdAlsHash->{$ids->[$i]}}; $i++) {
					push(@{$names},$cpdAlsHash->{$ids->[$i]}->[$i]->alias());
				}	
			}
			$row = {DATABASE => [$ids->[$i]],
					PKB => [$obj->pKb()],
...
Definition:
	Output:{} = FIGMODELgenome->getRastFeatureTable({
		genome => string:genome ID
	});
	Output:{
...
sub getRastGenomeData {
	...
	#Checking for rights to genome
	if ($self->figmodel()->user() eq "PUBLIC") {
		return $self->error_message({message=> "Must be authenticated to access model"});
	} else {#if (!defined($self->figmodel()->config("super users")->{$self->figmodel()->user()})) {
		my $haveRight = 0;
		...
		if ($haveRight == 0) {
			return $self->error_message({message=> "Do not have rights to requested genome"});	
		}
	}
	#Getting MGRAST genome data
	if ($args->{genome} =~ m/^444\d\d\d\d\./) {
		my $output = {
			source => "MGRAST",
			genome => $args->{genome},
			features => ModelSEED::FIGMODEL::FIGMODELTable::load_table($self->figmodel()->config("Metagenome directory")->[0].$args->{genome}.".tbl","\t","|",0,["ID","GENOME","ROLES","SOURCE"]),
			name => "Unknown",
			taxonomy => "Metagenome",
			size => 0,
	...
	#Getting RAST genome data
	my $output = {features => ModelSEED::FIGMODEL::FIGMODELTable->new(["ID","GENOME","ESSENTIALITY","ALIASES","TYPE","LOCATION","LENGTH","DIRECTION","MIN LOCATION","MAX LOCATION","ROLES","SOURCE","SEQUENCE"],"NONE",["ID","ALIASES","TYPE","ROLES","GENOME"],"\t","|",undef)};
	$output->{genome} = $args->{genome};
	$output->{source} = "RAST";
	my $job = $self->figmodel()->database()->get_object("rastjob",{genome_id => $args->{genome}});
	my $directory;
	if (!defined($job)) {
		$job = $self->figmodel()->database()->get_object("rasttestjob",{genome_id => $args->{genome}});
		if (!defined($job)) {
			return $self->error_message({message=> "Could not find RAST job object for genome".$args->{genome}});
		}
		$output->{source} = "TESTRAST";
		$directory = "/vol/rast-test/jobs/".$job->id();
	} else {
		$directory = "/vol/rast-prod/jobs/".$job->id();	
	}
	require FIGV;
	my $figv = new FIGV($directory."/rp/".$args->{genome});	
	if (!defined($figv)) {
		return $self->error_message({message=> "Could not create FIGV object for RAST genome:".$args->{genome}});
	}
	...
	$output->{source} .= ":".$job->id();
	$output->{owner} = $self->figmodel()->database()->load_single_column_file($directory."/USER","\t")->[0];
	$output->{activeSubsystems} = $figv->active_subsystems($args->{genome});
	my $completetaxonomy = $self->figmodel()->database()->load_single_column_file($directory."/TAXONOMY","\t")->[0];
